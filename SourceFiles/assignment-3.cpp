...
std::vector<int> FindAll(const std::string &source, const std::string &search_string,
    Case searchCase = Case::INSENSITIVE, size_t offset = 0 ) {
    ...
    for (int itr = offset;itr<source_length;itr++) {
        ...
        if (compareChars(source[itr2],search_string[search_itr], searchCase)) {
            ...
            if (leng == search_itr) ans.push_back(itr);
        }
    }
    return ans;
}
...
int main() {
    ...
    std::cout << FindAll("abcfghtabcjkjkabc", "abc", Case::INSENSITIVE, 0) << std::endl;
    return 0;
}