...
class thread_safe_queue {
    struct mintime {
        operator()(pair<int, T> &a, pair<int, T> &b) {
            return a.first > b.first;
        }
    };
    priority_queue<pair<int, T>, vector<pair<int, T>, mintime> q;
    mutex m;
public:
    int size() {
...
public:
    ...
    auto submit(long int seconds, T&& f, Args&& ...args) -> future<decltype(f(args...))> {
        ...
        auto current_time_in_seconds = duration_cast<chrono::seconds>(chrono::system_clock::now().time_since_epoch());

        task_list.enque(pair<int, function<void()>>(current_time_in_seconds+seconds, wrapper_func));
        cv.notify_one();
